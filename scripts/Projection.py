...
top_cryptos = [
    ("bitcoin", "BTC-USD Bitcoin"), ("ethereum", "ETH-USD Ethereum"), ("ripple", "XRP-USD XRP"),
    ("tether", "USDT-USD Tether USDt"), ("solana", "SOL-USD Solana"), ("binancecoin", "BNB-USD BNB"),
    ("usd-coin", "USDC-USD USD Coin"), ("dogecoin", "DOGE-USD Dogecoin"),
    ("staked-ether", "STETH-USD Lido Staked ETH"), ("cardano", "ADA-USD Cardano"),
    ("tron", "TRX-USD TRON"), ("wrapped-tron", "WTRX-USD Wrapped TRON"),
    ("hyperliquid", "HYPE32196-USD Hyperliquid"), ("wsteth", "WSTETH-USD Lido wstETH"),
    ("chainlink", "LINK-USD Chainlink"), ("wrapped-beacon-eth", "WBETH-USD Wrapped Beacon ETH"),
    ("weth", "WETH-USD WETH"), ("wrapped-bitcoin", "WBTC-USD Wrapped Bitcoin"),
    ("sui", "SUI20947-USD Sui"), ("usde", "USDE29470-USD Ethena USDe")
]
print("Top cryptocurrencies (CoinGecko ID, Yahoo Symbol):")
for cg_id, yahoo_name in top_cryptos:
    print(f"{cg_id}: {yahoo_name}")
...
cam_yaw = 0.0
cam_pitch = 0.0
move_speed = 0.05  # Slower movement
rotate_speed = 1.0  # Slower rotation
mouse_sensitivity = 0.05  # Smoother mouse rotation
points = None
point_colors = None
n_streams = None
x_dim = 0
...
def fetch_historical_yahoo(symbol, period1, period2, interval='1d', retry=False):
    ...
    try:
        ...
        if e.code == 429 and retry:
            print(f"Rate limited for {symbol}. Waiting {RETRY_DELAY} seconds and retrying...")
            time.sleep(RETRY_DELAY)
            return fetch_historical_yahoo(symbol, period1, period2, interval, retry=False)
...
def fetch_historical(symbol, period1, period2, interval='1d', retry=False):
    if USE_COINGECKO:
        yahoo_to_cg = {yahoo.split()[0]: cg_id for cg_id, yahoo in top_cryptos}
        cg_id = yahoo_to_cg.get(symbol)
        if not cg_id:
            print(f"No CoinGecko ID for {symbol}")
            return None
        return fetch_historical_coingecko(cg_id, period1, period2)
    else:
        return fetch_historical_yahoo(symbol, period1, period2, interval, retry)
...
def load_data(symbol_list, include_time, force_refresh=False):
    ...
    for i, sym in enumerate(symbol_list):
        df = fetch_historical(sym, period1, period2, interval, retry=True)
...
def initialize_projection():
    ...
    if points is not None and cam_dir is not None:
        ...
        mask = (depth > 0.1) & (depth < 1000)  # Additional clamp to avoid extreme projections
        if np.any(mask):
            factor = focal_length / depth[mask]
            factor = np.clip(factor, -1000, 1000)  # Prevent extreme scaling
    ...
    try:
                ...
                if setting_config:
                        ...
                        if event.key == pygame.K_RETURN:
                            try:
                                if active_box["label"] == "Symbols (e.g., BTC-USD,ETH-USD)":
                                    ...
                                    valid_symbols = [yahoo.split()[0] for _, yahoo in top_cryptos]
                    ...
                    if event.key == pygame.K_r and not (pygame.key.get_mods() & pygame.KMOD_SHIFT):
                        if load_data(symbols, include_time, force_refresh=True):
                            update_projection()
                    elif event.key == pygame.K_r and (pygame.key.get_mods() & pygame.KMOD_SHIFT):  # Shift+R for reset
                        cam_pos = np.zeros(n_streams)
                        cam_yaw = 0.0
                        cam_pitch = 0.0
                        focal_length = 100.0
        ...
        if not setting_config:
            ...
            if keys[pygame.K_s]:
                ...
                update = True
            if keys[pygame.K_a]:
                right = np.cross(cam_dir, np.array([0, 1, 0] + [0]*(n_streams-3)))
                norm = sqrt(sum(right**2))
                if norm > 0:
                    cam_pos -= (right / norm) * move_speed * speed_modifier
                    update = True
            if keys[pygame.K_d]:
                right = np.cross(cam_dir, np.array([0, 1, 0] + [0]*(n_streams-3)))
                norm = sqrt(sum(right**2))
        ...
        if setting_config:
            ...
            hint_text = "Available symbols: " + ", ".join([yahoo.split()[0] for _, yahoo in top_cryptos[:5]]) + "... (see console for full list)"
